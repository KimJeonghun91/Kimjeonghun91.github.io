...
- **RN core 업그레이드 문제** <br>
  ...
  CLI를 사용할 때는 써드파티 라이브러리를 도입할 때마다 네이티브 코드를 직접 추가해야 하는 경우가 많았다. 이런 네이티브 수정이 쌓이면 RN 버전을 올릴 때마다 비용이 커지고, 라이브러리 업데이트로 네이티브 코드가 바뀌면 최신 문서를 다시 확인하면서 수동으로 맞춰야 했다.
  ...
  Expo로 넘어온 이후에는 써드파티에서 config plugin 형태로 네이티브 변경을 주입해주는 경우가 많아져, 네이티브 유지보수 관점에서 충분히 유의미한 전환이었다.
...
글 쓰기가 귀찮아서 이틀 동안 내가 쓴 블로그 글과 커밋 내역으로 AI agent에게 "나"처럼 글을 작성하라고 AI를 갈궈봤다. 하지만 AI는 내가 만족할 만큼 "나"를 표현하지는 못하더라.
