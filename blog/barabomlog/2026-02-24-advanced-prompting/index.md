...
즉, 라우팅의 기준이 단순 키워드 매칭이 아니라 **대화 상태(state)**가 되었다.
...
운영 관점에서 중요한 점은 타임아웃 시에도 "응답 없음"으로 끝내지 않는 것이다. 핵심 답변 또는 재시도 안내를 포함한 fallback으로 사용자 경험을 보존한다.
...
이번 개선의 핵심은 모델을 "좋은 것"으로 바꾼 게 아니라, **시스템을 망가지기 어렵게 설계한 것**이다.
